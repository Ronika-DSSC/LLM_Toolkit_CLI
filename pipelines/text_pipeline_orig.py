...
def process(file_path: str, file_type: str ,run_id: str, llm_model: str | None = None, temperature: float | None = None, max_tokens: int | None = None,) -> dict[str, Any]:
    ...
    if file_type == "csv":
        return _process_csv(file_path, run_id, client)

    raw_text, metadata = _extract_raw_text(file_path, file_type)
    mechanically_cleaned = _mechanical_clean(raw_text)

    llm_result = client.clean_and_score_text(mechanically_cleaned, file_type)
    final_text = llm_result["cleaned_text"] or mechanically_cleaned
...
def _process_csv(file_path: str, run_id: str, llm_client,) -> dict[str, Any]:
    ...
    try:
            result = llm_client.clean_table(structural_md)
            candidate = result["cleaned_table"]
            valid, reason = _validate_cleaned_table(candidate, header, num_rows)
            if valid:
                final_md = candidate
                model = result["model"]
                note = "LLM-cleaned, shape validated"
            else:
                note = f"LLM output rejected ({reason}) — structural parse kept"
    except Exception as e:  # noqa: BLE001 — cleaning is best-effort
            note = f"LLM cleaning failed ({type(e).__name__}) — structural parse kept"

    confidence = _score_csv(ragged_ratio, llm_cleaned=(final_md is not structural_md))
